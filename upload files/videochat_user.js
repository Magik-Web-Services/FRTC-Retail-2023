...

{
            ...
            if (easyrtc.idToName(VideoChat.MyChatId) === VideoChat.CurrentRoomName) {
                ...
                $("#selfVideo").removeClass("hidden");
					...
					$.ajax({
						...
						url: "https://m.buildsite1.info/token_message.php",
    ...
    ConnectStreamer: function () {
        ...
        easyrtc.initMediaSource(
            ...
            function () { // success callback
                ...
                easyrtc.connect("VideoChatProd", VideoChat.HTML_LoginSuccess, VideoChat.HTML_LoginFailure);
    ...
    ConnectUser: function () {
       ...
       // easyrtc.enableCamera(false, stream);
        //easyrtc.enableMicrophone(false, stream);

        // disable prompt for mic and camera use
    ...
    ConnectUserForPrivateChat: function () {
        ...
        if (VideoChat.PrivateChatMode === 2) {
            ...
            // prompt user to use camera and mic
            easyrtc.initMediaSource(

                function () { // success callback

                    // if camera and mic available, connect to private chat and show user video
                    easyrtc.setVideoObjectSrc(document.getElementById("selfVideo"), easyrtc.getLocalStream());
            ...
            //spectator mode
            // block camera prompt and audio receiving for spectators
            ...
            //admin mode - similar to viewing as guest, no camera prompt
            easyrtc.enableAudio(false);
            easyrtc.enableAudioReceive(true);
            easyrtc.enableVideo(false);
            easyrtc.enableVideoReceive(true);
    ...
    UpdateStreamerStatus: function (value) {
        ...
        if (value === 2) {
            ...
            if (VideoChat.PrivateChatMode === 0) {
                ...
                if(VideoChat.ActiveUser){
                    ...
                    if(VideoChat.ActiveUser.tokens > VideoChat.model.scpm ){
                        ...
                        VideoChat.HTML_NotificationOneButton("<b>" + easyrtc.idToName(VideoChat.Streamer) + "</b> is in a private chat!",
            ...
            } else {//if( easyrtc.getConnectStatus(VideoChat.Streamer) == easyrtc.NOT_CONNECTED ){
    ...
    HTML_CallFailure: function (errorCode, errorText) {
        ...
        console.log("<---------- HTML_CallFailure");        console.log(errorCode + errorText);
	...
	$.ajax({
        ...
        url: "https://m.buildsite1.info/rtc/gettocken.php?username="+easyrtc.idToName(easyrtcid),
	...
	if(tokenss != ''){
		...
		color = 'red';
            ...
            if ((easyrtc.idToName(VideoChat.MyChatId) === VideoChat.CurrentRoomName && VideoChat.PrivateChatMode !== 2) || (easyrtc.idToName(easyrtcid) === VideoChat.CurrentRoomName && VideoChat.ActiveUser && VideoChat.PrivateChatMode === 0) ) {
                ...
                html += "<a href=\"#\" style='color:"+color+";' class=\"chatUser\" onclick=\"return VideoChat.SelectUser('" + easyrtcid + "');\"><i style='color:"+color+";' class=\"fa fa-user" + (easyrtc.idToName(easyrtcid) !== easyrtcid ? "" : "-secret") + "\" title=\"" + easyrtc.idToName(easyrtcid) + "\"></i> " + easyrtc.idToName(easyrtcid) + "</a>";

            } else {

                html += "<a href=\"#\" style='color:"+color+";' style='color:"+color+";' class=\"chatUser\"><i class=\"fa fa-user" + (easyrtc.idToName(easyrtcid) !== easyrtcid ? "" : "-secret") + "\" title=\"" + easyrtc.idToName(easyrtcid) + "\"></i> " + easyrtc.idToName(easyrtcid) + "</a>";
    ...
    HTML_AddToConversation: function (from, type, content, to) {
        ...
        console.log("<---------- HTML_AddToConversation :: " + type + content);console.log('hmm');console.log(type);

        if (type === "satus_pc") {            console.log('hmm'); VideoChat.UpdateStreamerStatus(parseInt(content));         }

        else if (type === "msg") { // CHAT MESSAGE
...
//	if(easyrtc.idToName(from) != 'Me'){
	...
	$.ajax({
        ...
        url: "https://m.buildsite1.info/rtc/gettocken.php?username="+easyrtc.idToName(from),
	...
	if(tokenss != ''){
		...
		color = 'red';
            ...
            html += "<b id='"+color+"' class=\"from\">" + (from === "Me" ? from : easyrtc.idToName(from)) + "</b>" + (to.targetEasyrtcid && VideoChat.PrivateChatMode !== 2 ? " to <b class=\"to\">" + easyrtc.idToName(to.targetEasyrtcid) + "</b>" : "") + " : " + content;
...
//	}
				...
				get_user_gender(easyrtc.idToName(from));
			...
			window.location.href = "https://m.buildsite1.info/";
            ...
            html += "<span class='boldit' style='font-weight: 600;'>" + easyrtc.idToName(from) + "</span> tipped " + content + " tokens";
					...
					setTimeout(function() {
						...
						$.ajax({
							...
							success: function (data) {
								...
								window.location.href = "https://m.buildsite1.info/cp/chatusers/buyminutes.php";
...
function VideoAcceptRequest() {
	...
	$.ajax({
            ...
            url: "https://m.buildsite1.info/cp/chatmodels/button_action.php",
...
function sendmessagesdb(getmessages) {
	...
	$.ajax({
            ...
            url: "https://m.buildsite1.info/send_messages.php",
...
function get_user_gender(get_userss){
	...
	$.ajax({
            ...
            url: "https://m.buildsite1.info/get_usser_gender.php",
