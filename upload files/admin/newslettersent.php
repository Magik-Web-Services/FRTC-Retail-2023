		...
		<td bgcolor="#FBFBFB">
			...
			<table width="600" border="0" align="center">
				<tr>
					<td class="big_title">
						<div align="center">
							<h2>Your news letter has been sent...</h2> <br>
							<br>
							<?php

							include('../dbase.php');
							include('../settings.php');
							if (isset($_POST['members1']) == "true") {
								echo '<span class="big_title">Free Acess Members</span><br><span class="form_informations">(';
								$result = mysqli_query($conn, 'SELECT email,user from chatusers WHERE money<=0');
							...
							if (isset($_POST['models']) == "true") {
								echo '<span class="big_title">Models</span><br><span class="form_informations">(';
								$result = mysqli_query($conn, 'SELECT email,user from chatmodels WHERE status!="rejected" AND status!="pending"');
								while ($row = mysqli_fetch_array($result)) {
									$email = $row['email'];
									$user = $row['user'];
									$charset = "Content-type: text/plain; charset=iso-8859-1\r\n";
							...
							if (isset($_POST['sop']) == "true") {
								echo '<span class="big_title">Studio Operators</span><br><span class="form_informations">(';
								$result = mysqli_query($conn, 'SELECT email,user from chatoperators');
								while ($row = mysqli_fetch_array($result)) {
									$email = $row['email'];
									$user = $row['user'];
									$charset = "Content-type: text/plain; charset=iso-8859-1\r\n";
