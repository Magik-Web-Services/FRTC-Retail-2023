...
if (isset($_POST['hiddenField']) == "yes" && $_POST['email'] == "true") {

	mysqli_query($conn, "UPDATE chatusers SET emailnotify='1' WHERE user='$username'");

	$temail = "1";
} else if (isset($_POST['hiddenField']) == "yes" && $_POST['email'] == "") {

	mysqli_query($conn, "UPDATE chatusers SET emailnotify='0' WHERE user='$username'");

	$temail = "0";
}

if (isset($_POST['hiddenField']) == "yes" && $_POST['sms'] == "true") {

	mysqli_query($conn, "UPDATE chatusers SET smsnotify='1' WHERE user='$username'");

	$tsms = "1";
} else if (isset($_POST['hiddenField']) == "yes" && $_POST['sms'] == "") {

	mysqli_query($conn, "UPDATE chatusers SET smsnotify='0' WHERE user='$username'");
...
if ($temail == "") {
	...
	$result = mysqli_query($conn, "SELECT emailnotify from chatusers WHERE user='$username' LIMIT 1");
...
if ($tsms == "") {
	...
	$result = mysqli_query($conn, "SELECT smsnotify from chatusers WHERE user='$username' LIMIT 1");
...
if (isset($_GET['remove']) && $_GET['remove'] != "") {
	...
	mysqli_query($conn, "DELETE from favorites WHERE model='$_GET[remove]' AND member='$username' LIMIT 1");
}
	...
	<div class="user_favorites_profile_seciton_sec">
		<div class="sub_seciton_favorites">
			<div class="col-md-12 no-padding">
				...
				$result = mysqli_query($conn, "SELECT t1.*, t2.* FROM favorites AS t1,chatmodels AS t2 WHERE t1.member='$username' AND t2.user=t1.model AND t2.status!='pending' AND t2.status!='offline' AND t2.status!='rejected' $modelsd order by rand()");
						...
						<div class="backgorund_urll">
							<div class="backgorund_urll12">
								...
								<span class="modelbox_title <?php echo $femacls; ?>">
									...
									<a href="favorites.php?remove=<?php echo $username; ?>">
										<h3>Remove</h3>
									</a>
								</span>
							</div>
