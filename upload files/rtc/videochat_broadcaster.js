...
var VideoChat =
{
    ...
    Init: function () {
        ...
        // Chose a secure password and eneter it below, it will //
		// act as your personal identifier, do not give this to ANYBODY! //
...
//        easyrtc.setVideoObjectSrc(document.getElementById('streamerVideo'), "");
   ...
   //     easyrtc.joinRoom(VideoChat.ActiveUser.Username);
        ...
        connection.open(udata+''+connection.publicRoomIdentifier, function (isRoomOpened, roomid, error) {
            ...
            VideoChat.HTML_LoginSuccess(udata);
...
//        }
    ...
    ToggleMicro: function () {
      ...
      connection.getUserMedia((e) => {
		...
		/*	$(".mic i").removeClass().addClass('fa fa-microphone-slash'); */
			$(".mic").css("color", "#f98585");
        } else {
            
            VideoChat.micEnabled = true;
		/*	$(".mic i").removeClass().addClass('fa fa-microphone'); */
...
//            });
               ...
               var html = "<li user-id=\"" + easyrtcid + "\" class=\"" + (VideoChat.PrivateChatMode === 2 && VideoChat.ActiveUser.IsStreamer && VideoChat.SelectedUserId === easyrtcid ? "selected privateChat " : "") + (fname === VideoChat.CurrentRoomName ? "streamerIcon" : "") + "\">";
                    ...
                    if ((getFullName(VideoChat.MyChatId) === VideoChat.CurrentRoomName && VideoChat.PrivateChatMode !== 2) || (fname === VideoChat.CurrentRoomName && VideoChat.ActiveUser && VideoChat.PrivateChatMode === 0)) {
                        html += "<a href=\"#\" style='color:" + color + ";' class=\"chatUser\" onclick=\"return VideoChat.SelectUser('" + easyrtcid + "');\"><i style='color:" + color + ";' class=\"fa fa-money" + (fname !== easyrtcid ? "" : "-secret") + "\" title=\"" + fname + "\"></i> " + fname + "</a>";
                    } else {
                        html += "<a href=\"#\" style='color:" + color + ";' style='color:" + color + ";' class=\"chatUser\"><i class=\"fa fa-money" + (fname !== easyrtcid ? "" : "-secret") + "\" title=\"" + fname + "\"></i> " + fname + "</a>";
                    }

                    if (getFullName(VideoChat.MyChatId) === VideoChat.CurrentRoomName) {
                        html += "&nbsp;&nbsp;<a href=\"#\" onclick=\"return VideoChat.KickUser('" + easyrtcid + "');\"><i class=\"fa fa-gavel\"></i></a>";
                    }
                    html += "</li>";
                    if (fname === VideoChat.CurrentRoomName) {
                        $("#users").prepend(html);
    ...
    HTML_AddToConversation: function (from, type, content, to, fromid) {
        ...
        if (type === "msg") { // CHAT MESSAGE
            ...
            $(".requestPrivateChat span").text("End Private Session");
          ...
          //  easyrtc.disconnect();
            ...
            html += "<span class='boldit' style='font-weight: 600;'>" + from + "</span> tipped " + content + " tokens";
            html += "</li>";
