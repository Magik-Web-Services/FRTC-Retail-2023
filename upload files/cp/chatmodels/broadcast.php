...
$result12 = mysqli_query($conn, "SELECT * from chatmodels WHERE id='" . $_COOKIE['id'] . "' LIMIT 1");
...
$mediaUrl = mysqli_fetch_array(mysqli_query($conn, $sql))['value'];
?>
  <?php
  ...
  $nMoney=$nMoney-$nMoneySent;
    ...
    $result = mysqli_query($conn, 'SELECT id,user,cpm,epercentage,lovense FROM chatmodels WHERE id="' . $_COOKIE["id"] . '" LIMIT 1');
  ...
  <tr>
                    ...
                    <div class="max-video-w">
                        ...
                        <div class="refreshh1244">
                            <div class="tip-sectionsa">
                                <?php
                                $relt22 = mysqli_query($conn, "SELECT SUM(cpm), MAX(cpm) FROM videosessions_copy WHERE model='$model' ORDER BY date DESC");
                                ...
                                $relt12202 = mysqli_query($conn, "SELECT cpm as cptoken, member as latst_tokn_mmbr_nm FROM videosessions_copy WHERE model='$model' ORDER BY date DESC");
                                ...
                                $relt220 = mysqli_query($conn, "SELECT member FROM videosessions_copy WHERE cpm='$max_tip' ORDER BY date DESC LIMIT 1");
                                while ($row121220 = mysqli_fetch_array($relt220)) {
                                    $member_name = $row121220['member'];
                                }
                                $resulttipgoal = mysqli_query($conn, "SELECT tipgoal FROM chatmodels WHERE user='" . $model . "' LIMIT 1");
                                $rowtipgoal = mysqli_fetch_array($resulttipgoal);
                                ...
                                <div class="tip-sections">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            ...
                                            <span><?php if ($latest_tip == "") {
                                                        echo "0";
                                                    } else {
                                                        echo $latest_tip_member;
                                                        echo " (" . $latest_tip . ")";
                                                    } ?></span>
                                        </li>
                                    </ul>
    ...
    <script type="text/javascript">
        /* Database Logic */
        var Users = [{
            Username: "<?php echo $sUser; ?>",
            Password: "<?php echo $sUser; ?>",
            IsStreamer: true,
            ImageUrl: "/models/<?php echo $sUser; ?>/thumbnail.jpg",
            IsAdmin: false
        }];
        var udata = '<?= $sUser ?>';
